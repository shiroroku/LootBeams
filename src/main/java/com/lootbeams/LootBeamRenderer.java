...
public class LootBeamRenderer extends RenderType {
	...
	public static void renderLootBeam(PoseStack stack, MultiBufferSource buffer, float pTicks, long worldTime, ItemEntity ie) {
		float beamAlpha = Configuration.BEAM_ALPHA.get().floatValue();
		float entityTime = ie.tickCount;
		// Fade out when close
		if (Minecraft.getInstance().player.distanceToSqr(ie) < 2f) {
			beamAlpha *= Minecraft.getInstance().player.distanceToSqr(ie);
		}
		...
		if (Configuration.GLOW_EFFECT.get() && ie.isOnGround()) {
			stack.pushPose();
		...
		if (Configuration.RENDER_NAMETAGS.get()) {
			renderNameTag(stack, buffer, ie, color);
		}

		if (Configuration.PARTICLES.get() && (!Configuration.PARTICLE_RARE_ONLY.get() || compatRarityCheck(ie, false))) {
			renderParticles(pTicks, ie, (int) entityTime, r, g, b);
		}
	}

	public static boolean compatRarityCheck(ItemEntity item, boolean shouldRender) {
		return shouldRender || item.getItem().getRarity() != Rarity.COMMON || (ModList.get().isLoaded("apotheosis") && !ApotheosisCompat.getRarityName(item.getItem()).equals("common"));
	}
	...
	private static void renderParticles(float pTicks, ItemEntity item, int entityTime, float r, float g, float b) {
		...
		if (entityTime % particleCount == 0 && pTicks < 0.3f && !Minecraft.getInstance().isPaused()) {
			Double particleRadius = Configuration.PARTICLE_RADIUS.get();
			Double particleSpeed = Configuration.PARTICLE_SPEED.get();
			addParticle(ModClientEvents.GLOW_TEXTURE, r, g, b, 1.0f, Configuration.PARTICLE_LIFETIME.get(), RANDOM.nextFloat((float) (0.25f * Configuration.PARTICLE_SIZE.get()), (float) (1.1f * Configuration.PARTICLE_SIZE.get())),
					new Vec3(
							RANDOM.nextDouble(item.getX() - particleRadius, item.getX() + particleRadius),
							RANDOM.nextDouble(item.getY() - (particleRadius / 3f), item.getY() + (particleRadius / 3f)),
							RANDOM.nextDouble(item.getZ() -particleRadius, item.getZ() + particleRadius)),
					new Vec3(
							RANDOM.nextDouble(-particleSpeed / 2.0f, particleSpeed / 2.0f),
							RANDOM.nextDouble(particleSpeed),
							RANDOM.nextDouble(-particleSpeed / 2.0f, particleSpeed / 2.0f)));
		}
	}
